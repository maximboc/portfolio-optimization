...
def custom_tournament_selection(fitness, num_parents, ga_instance):
    ...
    for _ in range(num_parents):
        ...
        if f1 < f2:
            selected_parents.append(ga_instance.population[candidates[0]].copy())
        else:
            selected_parents.append(ga_instance.population[candidates[1]].copy())
...
def custom_best_selection(fitness, num_parents, ga_instance):
    ...
    return np.array([ga_instance.population[best_index].copy()])
...
def custom_threshold_selection(threshold):
    def custom_threshold_selection_(fitness, num_parents, ga_instance):
        ...
        selected_parents = ga_instance.population[selected_indices[:num_parents]].copy()
        return np.array(selected_parents)
