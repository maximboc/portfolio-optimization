...
def init_multi_model_display(num_assets: int):
    ...
    for model_name in models_dict.keys():
        selected_models[model_name] = st.sidebar.checkbox(
            f"Include {model_name}", model_name == "Sharpe Ratio"
        )
        ...
        if not final_tickers:
            ...
            for i, model_name in enumerate(selected_model_names):
                with st.spinner(f"Calculating {model_name} model..."):
                    ...
                    cur_model_risk = cur_risk
                    if model == "SLSQP":
                        cur_model_risk = risk_free_rate
                    ...
                    model_metrics[model_name] = calculate_model_metrics(
                        opti_returns_time, cur_model_risk
                    )
            ...
            st.subheader(
                f"💰 Portfolio Value Over Time (${money_amount:,.0f} Initial Investment)"
            )
            ...
            st.line_chart(
                stock_prices / stock_prices.iloc[0] * 100, use_container_width=True
            )
